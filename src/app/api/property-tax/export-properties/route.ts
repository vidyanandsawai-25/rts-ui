...
const logger = createLogger('ExportPropertiesRoute');
...
export async function GET(request: NextRequest): Promise<Response> {
  try {
    ...
    logger.info('[ExportPropertiesRoute] Requesting property export file', { endpoint, status, financeYearId });

    const upstream = await apiClient.fetch(endpoint);

    if (!upstream.ok) {
      const errText = await upstream.text().catch(() => 'Export failed');
      logger.error('[ExportPropertiesRoute] Backend returned error', {
    ...
    logger.error('[ExportPropertiesRoute] Unexpected server error', { error: error as Error });
