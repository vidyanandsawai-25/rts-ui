...
const logger = createLogger('AddTaxesExportPropertiesRoute');
...
export async function GET(request: NextRequest): Promise<Response> {
  try {
    ...
    logger.info('[AddTaxesExportPropertiesRoute] Proxying export request to backend', { endpoint, status, financeYearId });

    const upstream = await apiClient.fetch(endpoint);

    if (!upstream.ok) {
      const errText = await upstream.text().catch(() => 'Export failed');
      logger.error('[AddTaxesExportPropertiesRoute] Backend API error', {
    ...
    logger.error('[AddTaxesExportPropertiesRoute] Server error during export', { error: error as Error });
