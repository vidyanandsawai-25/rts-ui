...
import { resolvePendingPasswordResetOrRedirect } from '@/app/[locale]/login/forgot-password/actions';
import { ResetPasswordFormClient } from '@/components/modules/login/ResetPasswordFormClient';
...
export default async function ResetPasswordPage({ params }: Pick<LoginPageProps, 'params'>) {
  const { locale } = await params;
  setRequestLocale(locale);

  // Redirects to the request step if there is no verified reset token (expired, already used, or
  // a direct hit).
  await resolvePendingPasswordResetOrRedirect(locale);
        ...
        <div className="w-full max-w-md">
          <Card className="overflow-hidden rounded-2xl border border-white/40 bg-white/80 shadow-2xl backdrop-blur-md transition-all duration-500 ease-in-out">
            ...
            <CardContent className="space-y-4 px-6 pb-10 pt-4 sm:px-8">
              <ResetPasswordFormClient locale={locale} />
            </CardContent>
          </Card>
        </div>
      </div>
