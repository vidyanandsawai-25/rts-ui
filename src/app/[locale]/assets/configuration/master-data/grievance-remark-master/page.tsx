...
import { AssetGrievanceRemarkMaster } from "@/components/modules/assets/configuration/master-data/grievance-remark-master/AssetGrievanceRemarkMaster";
import { fetchAssetGrievanceRemarkPagedServerAction } from "./action";
...
const ALLOWED_SORT_COLUMNS = ["remark", "grievanceCategoryId"] as const;
...
export default async function Page({ searchParams }: PageProps): Promise<React.ReactElement> {
  ...
  const result = await fetchAssetGrievanceRemarkPagedServerAction(pageNumber, pageSize, searchTerm, sortBy, sortOrder);
  return (
    <AssetGrievanceRemarkMaster
      data={result.items}
