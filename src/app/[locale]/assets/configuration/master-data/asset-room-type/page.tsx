...
import { AssetRoomTypeMaster } from "@/components/modules/assets/configuration/master-data/asset-room-type-master/AssetRoomTypeMaster";
import { fetchAssetRoomPagedServerAction } from "./action";
...
const ALLOWED_SORT_COLUMNS = ["roomTypeCode", "roomTypeName", "description"] as const;
...
export default async function Page({ searchParams }: PageProps): Promise<React.ReactElement> {
  ...
  const result = await fetchAssetRoomPagedServerAction(pageNumber, pageSize, searchTerm, sortBy, sortOrder);
  return (
    <AssetRoomTypeMaster
      data={result.items}
