...
import { RtsServiceMaster } from "@/components/modules/rts/services/RtsServiceMaster";
import { fetchRtsServicesPagedAction } from "./action";
...
export default async function Page({ searchParams }: PageProps): Promise<React.ReactElement> {
  ...
  // Fetch departments + paged services in parallel
  const [departments, result] = await Promise.all([
    getAllRtsDepartments().catch(() => []),
    fetchRtsServicesPagedAction(pageNumber, pageSize, searchTerm),
  ]);
