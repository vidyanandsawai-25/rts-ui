...
import { RtsFieldMaster } from "@/components/modules/rts/fields/RtsFieldMaster";
import { fetchRtsFieldsPagedAction } from "./action";
...
export default async function Page({ searchParams }: PageProps): Promise<React.ReactElement> {
  ...
  // Fetch departments + paged fields in parallel
  const [departments, result] = await Promise.all([
    getAllRtsDepartments().catch(() => []),
    fetchRtsFieldsPagedAction(pageNumber, pageSize, searchTerm),
  ]);
