...
describe("useAssetRoomPagination", () => {
  const defaultProps = {
    pageNumber: 1,
    pageSize: 10,
    totalCount: 100,
  ...
  describe("buildUrl", () => {
    it("should build URL with page and pageSize", () => {
      const { result } = renderHook(() => useAssetRoomPagination(defaultProps));

      const url = result.current.buildUrl(2, 20);

      expect(url).toContain("/en/assets/configuration/master-data/asset-room-type");
