 ...
 */
export function validateCreateFormData(data: MoujaFormModel): void {
  if (!data.moujaNo?.trim()) {
    throw new ApiError(400, "Mouja number is required", "Validation failed");
  }
  if (!data.moujaName?.trim()) {
    throw new ApiError(400, "Mouja name is required", "Validation failed");
  }
}
 ...
 */
export function validateUpdateFormData(data: MoujaFormModel): void {
  if (!data.id || data.id <= 0) {
    throw new ApiError(400, "Mouja ID is required for update", "Validation failed");
  }
  if (!data.moujaNo?.trim()) {
    throw new ApiError(400, "Mouja number is required", "Validation failed");
  }
