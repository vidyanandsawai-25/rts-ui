...
export async function bulkCreateAgeFactorCVMaster(
  ...
  try {
    ...
    return await apiClient.post<unknown>('/asset-management/age-factor-cv/Bulk', requestPayload);
  } catch (error) {
    throw error;
  }
}
...
export async function bulkUpdateAgeFactorCVMaster(
  ...
  try {
    ...
    const response = await apiClient.put<unknown>(
      '/asset-management/age-factor-cv/Bulk',
      requestPayload
    );
...
export async function bulkDeleteAgeFactorCVMaster(
  ...
  return apiClient.delete<unknown>('/asset-management/age-factor-cv/Bulk', { body: JSON.stringify({ ids }) });
}
