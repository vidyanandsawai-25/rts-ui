...
import { getErrorStatusCode } from '@/lib/utils/backend-error-detection';
...
export async function bulkCreateUseFactorCVMaster(
  ...
  try {
    ...
    const response = await apiClient.post<unknown>('/asset-management/use-factor-cv/Bulk', requestPayload);
...
export async function bulkUpdateUseFactorCVMaster(
  ...
  try {
    ...
    const response = await apiClient.put<unknown>(
      '/asset-management/use-factor-cv/Bulk',
      requestPayload
    );
