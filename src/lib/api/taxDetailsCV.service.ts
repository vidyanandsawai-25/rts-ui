...
/* ============================================================
   TAX DETAILS — SERVICE
   Base endpoint: /Property/{propertyId}/tax-details-cv
============================================================ */

/**
 * Fetch tax details for a specific property.
 *
 * @param propertyId - The property ID to fetch tax details for.
 * @returns The full API response envelope containing tax amounts.
 *
 * @example
 * const res = await getTaxDetailsByPropertyId(549453);
 * const taxAmounts = res.data?.items?.taxAmounts;
 */
export async function getTaxDetailsByPropertyId(
  propertyId: string | number
): Promise<ApiResponse<TaxDetailsResponse>> {
  const endpoint = `/Property/${propertyId}/tax-details-cv`;
...
/**
 * Convenience wrapper that fetches and maps tax details to table rows.
 * Returns empty array and zero totals on failure.
 *
 * @param propertyId - The property ID to fetch tax details for.
 * @returns Object containing taxRows array, totalRV, and totalTax values.
 */
export async function getTaxDetailsForTable(
  propertyId: string | number
): Promise<{ taxRows: TaxRow[]; totalRV: number; totalTax: number }> {
  try {
    const response = await getTaxDetailsByPropertyId(propertyId);

    if (!response.success || !response.data?.items?.taxAmounts) {
      console.warn('[taxDetails.service] API returned failure:', response.error);
    ...
    console.error('[taxDetails.service] Unexpected error:', err);
    return { taxRows: [], totalRV: 0, totalTax: 0 };
  }
}
