...
function buildPieData(
  ...
  const visibleRows =
    otherValue > 0 ? [...topRows, { label: otherLabel, value: otherValue }] : topRows;
...
function DonutChart({
  ...
  if (loading) {
    return (
      <div className="flex h-56 items-center justify-center rounded-xl border border-dashed border-slate-200 bg-slate-50 px-4 text-center text-xs font-bold text-slate-500">
        {loadingLabel}
      </div>
    );
  }
        ...
        <svg viewBox="0 0 180 180" className="h-56 w-full" role="img" aria-label={totalLabel}>
          ...
          {data.map((item, index) => {
            ...
            const offset = data
              .slice(0, index)
              .reduce(
                (totalLength, previousItem) =>
                  totalLength + (previousItem.value / total) * DONUT_CIRCUMFERENCE,
                0
              );
            return (
              <circle
          ...
          <text x="90" y="84" textAnchor="middle" className="fill-slate-500 text-[10px] font-bold">
            {totalLabel}
          </text>
          <text
            x="90"
          ...
          return (
            ...
            >
              ...
              <span className="shrink-0 text-slate-900">
                {formatNumber(item.value)} ({formatNumber(percentage)}%)
              </span>
            </div>
          );
        })}
...
export default function RtsMisDashboard({
  misDashboardData,
  getDepartmentServices,
  filters,
}: DashboardProps) {
  ...
  const initialDepartmentIdsByName = useMemo(
    () =>
      new Map(
        (misDashboardData.departmentWiseData ?? [])
          .filter(
            (department) => department.departmentId != null && department.departmentName?.trim()
          )
          .map((department) => [
    ...
    () =>
      (dashboardData.departmentWiseData ?? []).map((department, index) => {
        ...
        return {
          ...
          reverted: Number(department.reverted ?? 0),
  ...
  const compareMisRows = useCallback(
    (first: Pick<DepartmentRow, MisSortKey>, second: Pick<DepartmentRow, MisSortKey>) => {
      if (!sort) return 0;

      let result: number;
      if (sort.key === 'name') {
        result = new Intl.Collator(locale, { numeric: true, sensitivity: 'base' }).compare(
          first.name,
          second.name
        );
      } else {
    ...
    (key: MisSortKey, label: string) => {
      ...
      return (
        ...
        >
          ...
          <Icon
            aria-hidden
            className={`size-3 shrink-0 ${isActive ? 'opacity-100' : 'opacity-60 group-hover:opacity-100'}`}
          />
        </button>
      );
    },
    ...
    () =>
      departments.reduce(
        (current, department) => ({
          ...
          reverted: current.reverted + department.reverted,
          overdue: current.overdue + department.overdue,
        }),
        { total: 0, pending: 0, approved: 0, rejected: 0, reverted: 0, overdue: 0 }
      ),
    [departments]
  );
  ...
  const departmentPageStart =
    filteredDepartments.length === 0 ? 0 : (currentDepartmentPage - 1) * DEPARTMENT_PAGE_SIZE + 1;
  const departmentPageEnd = Math.min(
    currentDepartmentPage * DEPARTMENT_PAGE_SIZE,
    filteredDepartments.length
  );

  const fetchDepartmentServices = useCallback(
    async (department: DepartmentRow) => {
      if (servicesByDepartment[department.id] || activeServiceRequests.current.has(department.id))
        return;
      ...
      if (!Number.isFinite(numericId)) {
        setServiceErrors((current) => ({
          ...current,
          [department.id]: t('misDashboard.serviceLoadFailed'),
        }));
        return;
      }
  ...
  const metrics = [
    {
      ...
      icon: FileText,
      colors: 'bg-blue-50 text-[#0B5CD5] ring-blue-100',
      detailColor: 'text-[#0B5CD5]',
      statusFilter: null,
    },
    {
      ...
      icon: Clock3,
      colors: 'bg-amber-50 text-[#F39C12] ring-amber-100',
      detailColor: 'text-[#C66922]',
    ...
    },
    {
      ...
      icon: CheckCircle2,
      colors: 'bg-emerald-50 text-[#27AE60] ring-emerald-100',
      detailColor: 'text-[#0F7A3F]',
    ...
    },
    {
      ...
      icon: XCircle,
      colors: 'bg-rose-50 text-[#B22222] ring-rose-100',
      detailColor: 'text-[#B22222]',
      statusFilter: 'Rejected',
    },
    {
      count: totals.reverted,
      label: t('misDashboard.revertedApplications'),
      detail: t('misDashboard.requiresAttention'),
      icon: CornerDownRight,
      colors: 'bg-orange-50 text-orange-600 ring-orange-100',
    ...
    },
    {
      ...
      detail: t('misDashboard.requiresAttention'),
      icon: Timer,
      colors: 'bg-violet-50 text-[#8A2BE2] ring-violet-100',
      detailColor: 'text-[#551A8B]',
  ...
  const visualizationSummary = expandedDepartment
    ? {
        ...
        reverted: expandedDepartment.reverted,
        overdue: expandedDepartment.overdue,
      }
    : {
  ...
  const approvalDistribution = [
    {
      label: t('misDashboard.approved'),
      value: visualizationSummary.approved,
      color: 'bg-[#0F7A3F]',
      text: 'text-[#0F7A3F]',
    },
    {
      label: t('misDashboard.pending'),
      value: visualizationSummary.pending,
      color: 'bg-[#C66922]',
      text: 'text-[#C66922]',
    },
    {
      label: t('misDashboard.rejected'),
      value: visualizationSummary.rejected,
      color: 'bg-[#B22222]',
      text: 'text-[#B22222]',
    },
    {
      label: t('misDashboard.reverted'),
      value: visualizationSummary.reverted,
      color: 'bg-orange-600',
      text: 'text-orange-600',
    },
    {
      label: t('misDashboard.overdueCount'),
      value: visualizationSummary.overdue,
      color: 'bg-rose-700',
      text: 'text-rose-700',
    },
  ];
    ...
    () =>
      buildPieData(
        departments.map((department) => ({
          ...
          value: department.totalApplications,
        })),
  ...
  const isServicePieLoading = Boolean(
    expandedDepartment && loadingDepartmentId === expandedDepartment.id
  );
  ...
  const tableRows = useMemo<MisTableRow[]>(() => {
    return paginatedDepartments.flatMap((department) => {
      ...
      if (loadingDepartmentId === department.id || (!services && !error)) {
        rows.push({
          id: `${department.id}-loading`,
          kind: 'loading',
          srNo: null,
          name: t('misDashboard.loadingServices'),
          totalServices: null,
          totalApplications: null,
          fromRts: null,
          fromAapleSarkar: null,
          pending: null,
          approved: null,
          rejected: null,
          reverted: null,
        ...
        if (statusServices.length === 0) {
          rows.push({
            ...
            kind: 'empty',
            srNo: null,
            name: t('misDashboard.noServicesFound'),
            totalServices: null,
            totalApplications: null,
            fromRts: null,
            fromAapleSarkar: null,
          ...
          const sortedServices = sort ? [...statusServices].sort(compareMisRows) : statusServices;
          rows.push(
  ...
  const tableColumns = useMemo<Column<MisTableRow>[]>(() => {
    ...
    const metricColumn = (
      key: Exclude<MisSortKey, 'srNo' | 'name'>,
      label: string,
      width: string,
      cellClassName = ''
    ): Column<MisTableRow> => ({
      ...
      render: (value, row) =>
        row.kind === 'department' || row.kind === 'service' ? numberCell(value) : '',
    });

    return [
      {
        key: 'srNo',
        label: sortableHeader('srNo', t('misDashboard.srNo')),
        width: '56px',
        align: 'center',
        headerClassName: 'bg-[#0A3275] text-white text-[11px] font-bold',
        cellClassName: 'text-center font-extrabold',
        render: (value, row) => {
      ...
      },
      {
        ...
        render: (_value, row) => {
          if (row.kind === 'service')
            return (
              <span className="flex items-start gap-2 break-words pl-2 text-left leading-5 text-slate-700">
                <CornerDownRight className="mt-0.5 h-3.5 w-3.5 shrink-0 text-blue-400" />
                <span>{row.name}</span>
              </span>
            );
          if (row.kind === 'loading' || row.kind === 'empty')
            return <span className="block break-words pl-5 text-slate-500">{row.name}</span>;
          if (row.kind === 'error' && row.department)
            return (
              <div className="flex items-center gap-3 pl-5 text-rose-600">
                <span>{row.error}</span>
                <Button
                  type="button"
                  size="sm"
                  className="border border-slate-200 bg-white text-slate-700 hover:bg-slate-50"
                  onClick={() => fetchDepartmentServices(row.department!)}
                >
                  {t('misDashboard.retry')}
                </Button>
              </div>
            );
          ...
          return (
            <div className="flex items-start gap-2">
              ...
              <span
                title={`${formatNumber(row.totalServices ?? 0)} ${t('misDashboard.totalServices')}`}
                className="inline-flex h-5 min-w-5 ml-3 shrink-0 items-center justify-center rounded-full border border-blue-200 bg-blue-50 px-1.5 text-[10px] font-extrabold text-[#0B5CD5]"
              >
                {formatNumber(row.totalServices ?? 0)}
              </span>
            </div>
          );
        },
      ...
      metricColumn(
        'totalApplications',
        t('misDashboard.totalApplications'),
        '120px',
        'font-extrabold'
      ),
      ...
      {
        key: 'reverted',
        label: sortableHeader('reverted', t('misDashboard.reverted')),
        width: '90px',
        align: 'center',
        headerClassName: 'bg-[#0A3275] text-white text-[11px] font-bold',
        cellClassName: 'text-center font-bold text-orange-600',
        render: (value, row) =>
          row.kind === 'department' || row.kind === 'service' ? numberCell(value) : '',
      },
      ...
      {
        key: 'sla',
        label: sortableHeader('sla', t('misDashboard.avgSla')),
        width: '95px',
        align: 'center',
        headerClassName: 'bg-[#0A3275] text-white text-[11px] font-bold',
        cellClassName: 'text-center font-extrabold text-[#008B8B]',
        render: (value, row) =>
          row.kind === 'department' || row.kind === 'service'
            ? t('misDashboard.daysValue', { value: numberFormatter.format(Number(value)) })
            : '',
      },
    ];
  }, [expandedDepartment, fetchDepartmentServices, formatNumber, numberFormatter, sortableHeader, t]);
    ...
    <div className="space-y-3">
      <Card className="flex flex-col justify-between gap-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm sm:flex-row sm:items-center">
        ...
        <div className="flex shrink-0 flex-wrap items-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-3 py-1.5 shadow-sm">
          ...
          <Input
            ...
            className="h-8 min-w-[132px] border-0 bg-transparent px-1 text-[12px] font-semibold shadow-none focus-visible:ring-0"
          />
          ...
          {(fromDate || toDate) && (
            <Button
              ...
              className="h-5 min-h-0 px-1 text-[10px] font-bold text-slate-400 hover:bg-rose-50 hover:text-rose-500"
            >
        ...
        {metrics.map((metric) => {
          ...
          return (
            <Card
              key={metric.label}
              padding="none"
              className={`rounded-2xl border bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md ${isActive ? 'border-blue-400 ring-2 ring-blue-100' : 'border-slate-200'}`}
            >
              <button
                type="button"
                onClick={() =>
              ...
              >
                <div
                  className={`flex h-12 w-12 shrink-0 items-center justify-center rounded-full ring-4 ${metric.colors}`}
                >
                ...
                <div className="min-w-0">
                  ...
                  <div className={`mt-2 text-[11px] font-bold ${metric.detailColor}`}>
                    {metric.detail}
                  </div>
                </div>
              </button>
      ...
      <div className="flex flex-col gap-2 lg:flex-row lg:items-start">
        <Card className="w-full overflow-hidden border border-slate-200 bg-white p-4 shadow-sm lg:w-[80%]">
          <div className="mb-3 flex flex-col gap-2 border-b border-slate-100 pb-2 sm:flex-row sm:items-center sm:justify-between">
            <h2 className="flex items-center gap-2 text-sm font-bold text-[#0a3275]">
              ...
              {t('misDashboard.departmentServiceBreakdown')}
            </h2>
            <div className="flex flex-wrap items-center gap-2">
              <div className="w-auto flex flex-row items-center">
                <p className="text-[11px] w-auto text-right font-bold text-slate-500">
                  {t('misDashboard.applicationSource')}:{' '}
                </p>
                <Select
              ...
              <span className="rounded-md border border-emerald-200 bg-emerald-50 px-2.5 py-0.5 text-[9px] font-bold text-[#0F7A3F]">
          ...
          <MasterTable<MisTableRow>
            ...
            maxBodyHeightClassName="h-[558px]"
            tableClassName="table-fixed border-collapse text-left text-sm text-slate-900"
            containerClassName="gap-0"
            theadClassName="bg-[#0A3275]"
            rowClassName={(row) => {
              const minimumRowHeight = 'h-[50px]';
              if (row.kind === 'service')
                return `${minimumRowHeight} cursor-default bg-slate-50/90 hover:!bg-slate-100`;
              if (row.kind === 'loading' || row.kind === 'empty')
                return `${minimumRowHeight} cursor-default bg-slate-50 text-slate-500`;
            ...
            footerRightContent={
              <TablePagination
                ...
                onPageChange={changeDepartmentPage}
              />
            }
        ...
        <div className="w-full space-y-2 lg:w-[20%]">
          <Card className="flex h-[295px] flex-col justify-between gap-2 self-start border border-slate-200 bg-white p-3 shadow-sm">
            {/* Main header */}
            <div className="flex items-start justify-between gap-2">
              ...
              <div className="min-w-0 pt-1">
                <h3 className="flex items-center gap-0.5 text-base font-bold text-slate-800">
                  ...
                  <span>{expandedDepartment?.name ?? t('misDashboard.allDepartments')}</span>
                </h3>
                ...
                {approvalDistribution.map((item) => {
                  const percentage =
                    visualizationSummary.totalApplications > 0
                      ? Math.round((item.value / visualizationSummary.totalApplications) * 100)
                      : 0;
          ...
          <Card className="h-[400px] border border-slate-200 bg-white p-4 shadow-sm">
            <div className="mb-3 flex items-center justify-between gap-2 border-b border-slate-100 pb-2">
              <h3 className="flex items-center gap-1.5 text-sm font-bold text-[#0a3275] truncate">
                <PieChart className="h-4 w-4 text-[#0B5CD5]" />
                {t('misDashboard.applicationShare')}
              </h3>
            </div>
            <DonutChart
              data={activePieData}
