...
export default function RtsApplicationsDashboard({
  data,
  masters,
  locale,
}: RtsApplicationsDashboardProps) {
  const lang = locale === "mr" ? "mr" : "en";

  const [filterDept, setFilterDept] = useState("All");
  const [filterMonth, setFilterMonth] = useState("June 2026");
  const [searchTerm, setSearchTerm] = useState("");
  const [filterOutcome, _setFilterOutcome] = useState("All");
  ...
  console.log(filterOutcome); // to satisfy unused state warning
      ...
      >
        <div>
          <h1 className="text-xl font-bold tracking-tight text-slate-800">
            {lang === "en" ? "RTS Applications Dashboard" : "आरटीएस अर्ज डॅशबोर्ड"}
          </h1>
        ...
        <div className="flex flex-col justify-between gap-3 border-b border-slate-100 pb-3 sm:flex-row sm:items-center">
          <div>
            <h2 className="flex items-center gap-2 text-sm font-extrabold text-slate-900">
              ...
              {lang === "en"
                ? "APPLICATIONS & SLA TIMELINE"
                : "अर्ज आणि SLA कालमर्यादा"}
            </h2>
