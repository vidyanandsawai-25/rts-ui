...
import { pickLangText, safeLang } from '@/lib/utils/rts/lang';
...
export default function RtsApplicationDetails({
  ...
  const completedStepsCount = (application.timeline ?? []).filter(
  ...
  const currentStageIndex =
    workflowDetails && workflowDetails.stages.length > 0
      ...
      : -1;
  const currentStage: WorkflowStage | undefined =
    workflowDetails && currentStageIndex !== -1
  ...
  const defaultAction = currentStage
    ? currentStage.canApprove
      ? 'Approve'
      : currentStage.canVerifyDocument
        ? 'RequestDocuments'
        : currentStage.canReturn
          ? 'Return'
          : !currentStage.isFinalStage
            ? 'Forward'
            : currentStage.canReject
              ? 'Reject'
              : 'Hold'
    : 'Approve';

  const [actionType, setActionType] = useState<
    'Approve' | 'Reject' | 'Forward' | 'Return' | 'Hold' | 'RequestDocuments'
  >(defaultAction);
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between border-b border-slate-200 pb-5">
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-slate-400">
              Submitted on {application.appliedDate || application.submittedAt || '—'}
            </p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <div className="text-right text-xs">
            <p className="font-semibold text-slate-700">{application.serviceName}</p>
            <p className="text-[10px] text-slate-400">{application.departmentName}</p>
          </div>
        </div>
      </div>

      {/* SLA Status Card */}
      <Card className="p-4 bg-gradient-to-r from-blue-50/50 via-slate-50 to-teal-50/50 border border-slate-200/80">
        <div className="grid grid-cols-2 gap-4 sm:grid-cols-4">
          <div>
            ...
            <p className="text-xs font-bold text-slate-700 mt-0.5">
              {application.currentStage || currentStage?.stageName || 'Processing'}
            </p>
          </div>
          <div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
              Service SLA Timeline
            </p>
            <p className="text-xs font-bold text-slate-700 mt-0.5">
              {application.slaLimitDays || application.slaDays || 15} Days Total
            </p>
          </div>
          <div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
            ...
            <p
              className={`text-xs font-extrabold mt-0.5 ${
                (application.daysRemaining ?? 10) <= 2 ? 'text-red-600' : 'text-emerald-600'
              }`}
            >
              {application.daysRemaining ?? 10} Days
            </p>
          </div>
          <div>
            <p className="text-[10px] font-bold text-slate-400 uppercase tracking-wider">
      ...
      <div className="flex border-b border-slate-200 gap-6 text-xs font-bold">
        {[
          ...
          { id: 'form', label: 'Submitted Form Data', icon: FileCheck },
          { id: 'documents', label: 'Verification Checklist', icon: FileText },
          { id: 'timeline', label: 'Workflow & Audit Log', icon: History },
        ].map((tab) => {
          const Icon = tab.icon;
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {activeTab === 'profile' && (
            <Card className="p-6 space-y-6">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                <div>
                  <p className="text-slate-400 font-semibold">Full Applicant Name</p>
                  <p className="font-bold text-slate-800 mt-0.5">
                    {application.applicantName || application.citizenName || '—'}
                  </p>
                </div>
                <div>
                  <p className="text-slate-400 font-semibold">Mobile Number</p>
                  <p className="font-bold text-slate-800 mt-0.5">
                    {application.mobileNumber || application.citizenPhone || '—'}
                  </p>
                </div>
                <div>
                  <p className="text-slate-400 font-semibold">Email Address</p>
                  <p className="font-bold text-slate-800 mt-0.5">
                    {application.email || application.citizenEmail || '—'}
                  </p>
                </div>
                <div>
                  <p className="text-slate-400 font-semibold">Aadhaar (Masked)</p>
                  <p className="font-bold text-slate-800 mt-0.5">
                    {maskAadhaar(application.aadhaarNumber || '')}
                  </p>
                </div>
                <div className="sm:col-span-2">
                  <p className="text-slate-400 font-semibold">Residential / Premises Address</p>
                  <p className="font-bold text-slate-800 mt-0.5 whitespace-pre-wrap">
                    {application.address || application.citizenAddress || '—'}
                  </p>
                </div>
              </div>
            </Card>
          ...
          {/* TAB 2: Submitted Form Data */}
          {activeTab === 'form' && (
            <Card className="p-6">
              <div className="flex items-center justify-between border-b border-slate-100 pb-2 mb-4">
                <h3 className="text-sm font-bold text-slate-800">
                  Dynamic Service EAV Field Payload
                </h3>
              ...
              {!formSchema ? (
                <div className="flex flex-col items-center justify-center py-12 text-center text-slate-400 bg-slate-50 rounded-xl border border-dashed border-slate-200">
                  ...
                  <p className="text-xs font-semibold">
                    EAV Form layout metadata not configured in systems.
                  </p>
                  <p className="text-[10px] mt-1">
                    No generated steps were found inside the services registry.
                  </p>
                </div>
              ) : (
                <div className="space-y-6">
                  {formSchema.steps?.map((section: any, idx: number) => (
                    ...
                    >
                      <h4 className="text-xs font-bold text-[#4b70a6] uppercase tracking-wider mb-3 pb-1 border-b border-slate-100">
                        {idx + 1}.{' '}
                        {pickLangText(section.title, safeLang(locale)) || 'Section Details'}
                      </h4>
                      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
                        {section.fields?.map((field: any) => {
                          const val = application.fieldValues?.[field.id] || '—';
                          const label = pickLangText(field.label, safeLang(locale)) || 'Field';
                          ...
                          return (
                            <div key={field.id} className="text-xs">
                              ...
                              <p className="font-bold text-slate-800 mt-0.5 whitespace-pre-wrap truncate">
                                {field.type === 'select'
                                  ? pickLangText(
                                      field.options?.find((o: any) => o.value === val)?.label,
                                      safeLang(locale)
                                    ) || val
                                  : val}
                              </p>
          ...
          {activeTab === 'timeline' && (
            <div className="space-y-6">
                        ...
                        >
                          ...
                          <div className="mt-2 space-y-1 text-[10px] text-slate-500 font-semibold">
                            ...
                            <div className="flex flex-wrap gap-1 mt-1">
                              ...
                              {stage.canReject && (
                                <span className="bg-rose-100 text-rose-700 px-1 rounded">
                                  Reject
                                </span>
                              )}
                              {stage.canReturn && (
              ...
              {/* Movement History */}
              <Card className="p-6">
                <h3 className="text-sm font-bold text-slate-800 border-b border-slate-100 pb-2 mb-6">
                  Movement History
                </h3>
          ...
          <Card className="p-5 border border-[#4b70a6]/20 bg-slate-50/50 shadow-md sticky top-24">
            ...
            {currentStage && (
              <div className="mb-4 rounded-xl bg-blue-50/60 border border-blue-100 p-3 text-[11px] text-slate-700 space-y-1">
                <p className="font-extrabold text-[#4b70a6] uppercase tracking-wider text-[9px]">
            ...
            {['Approved', 'Rejected'].includes(application.status) ? (
              <div className="text-center py-6 text-slate-400 bg-slate-100/50 rounded-xl">
                ...
                <p className="text-[10px] mt-1 px-4">
                  No additional actions are allowed. Stored files are archived.
                </p>
              </div>
            ) : (
              <form onSubmit={handleActionSubmit} className="space-y-4">
                <div className="space-y-1">
                  ...
                  >
                    {!currentStage || currentStage.canApprove ? (
                      <option value="Approve">Approve & Issue Certificate</option>
                    ) : null}
                    {!currentStage || !currentStage.isFinalStage ? (
                      <option value="Forward">Forward to Next Official</option>
                    ) : null}
                    {!currentStage || currentStage.canReturn ? (
