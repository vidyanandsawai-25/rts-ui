...
export function GisMapCard({ image, onClick }: GisMapCardProps): React.ReactElement {
  ...
  return (
    <div
      className="relative group bg-slate-100 rounded-lg overflow-hidden border-2 border-slate-200 shadow-md hover:border-green-500 hover:border-4 transition-all cursor-pointer flex-1 min-h-[150px] lg:min-h-0"
      onClick={onClick}
    >
      <ImageWithFallback
        src={image?.src ?? ''}
