...
// import { PropertyPhotoToggle } from './appartmentQCDrawer/FloorSubmissionDrawer/PropertyPhotoToggle';
...
interface PropertyDetailsEditScreenProps {
	open: boolean;
	onClose?: () => void;
	onSaveOrClose?: () => void;
	propertyData?: ApartmentQCDetail | null;
...
export default function PropertyDetailsEditScreen({
	open,
	onClose,
	propertyData,
	subTab = 'rateable',
	...
	useTypes = [],
	allSubTypes = [],
}: PropertyDetailsEditScreenProps) {
	const t = useTranslations('appartmentQC');
	const [isBasicInfoOpen, setIsBasicInfoOpen] = React.useState(true);
	const [isFloorQCOpen, setIsFloorQCOpen] = React.useState(true);

	const translatedWard = t('drawer.wardLabel');
	const translatedZone = t('drawer.zoneLabel');
	const translatedProp = t('drawer.propLabel');

	const wardLabel = translatedWard === 'drawer.wardLabel' ? 'Ward' : translatedWard;
	const zoneLabel = translatedZone === 'drawer.zoneLabel' ? 'Zone' : translatedZone;
	const propLabel = translatedProp === 'drawer.propLabel' ? 'Prop' : translatedProp;

	const wardValue = String(propertyData?.wardNo || propertyData?.wardId || '-');
	const zoneValue = String(propertyData?.zoneNo || '-');
	const propValue = String(propertyData?.propertyNo || '-');

	if (!open) return null;

	return (
		<section data-testid="drawer" className="rounded-lg border border-slate-200 bg-white p-4">
			<h2 className="text-base font-semibold text-slate-800">{t('drawer.title')}</h2>
			<div className="mt-2 flex items-center gap-2 text-xs">
				<span>{`${wardLabel}: ${wardValue}`}</span>
				<span>{`${zoneLabel}: ${zoneValue}`}</span>
				<span>{`${propLabel}: ${propValue}`}</span>
			</div>
			<p className="mt-1 text-xs text-slate-600">{t('drawer.tabLabel')}: {subTab}</p>
			<p className="text-xs text-slate-600">{t('drawer.contextLabel')}: {returnTo}</p>
			<p className="text-xs text-slate-600">{t('drawer.propertyLabel')}: {String(propertyData?.id ?? '')}</p>
			<p className="text-xs text-slate-600">
				{t('drawer.lookupsLabel')}: {t('drawer.floorsLabel')} {floors.length}, {t('drawer.constructionLabel')} {constructionTypes.length}, {t('drawer.useLabel')} {useTypes.length}, {t('drawer.subUseLabel')} {allSubTypes.length}
			</p>
			<div className="mt-3 flex gap-2">
				<button type="button" onClick={onClose}>{t('drawer.cancel')}</button>
			</div>
			<div className="mt-3 space-y-2">
				<button type="button" onClick={() => setIsBasicInfoOpen((prev) => !prev)}>
					{t('drawer.basicInformation')}
				</button>
				{isBasicInfoOpen && <div className="text-xs text-slate-500">{t('drawer.basicInfoVisible')}</div>}
				<button type="button" onClick={() => setIsFloorQCOpen((prev) => !prev)}>
					{t('drawer.floorQC')}
				</button>
				{isFloorQCOpen && <div className="text-xs text-slate-500">{t('drawer.floorQCVisible')}</div>}
			</div>
		</section>
	);
}
