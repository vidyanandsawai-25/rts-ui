...
export const transformApartmentData = (items: ApartmentQCDetail[], _activeMainTab: string) => {
  if (_activeMainTab === 'commercial' || _activeMainTab === 'residential') {
    return items.map((item) => ({
      ...item,
      oldPropertyNo: (item.oldPropertyNo ?? '-') as string | null,
      ...
      oldConstArea: (item.oldConstArea ?? '-') as unknown as number,
      oldRV: (item.oldRV ?? '-') as unknown as number,
      oldTotalTax: (item.oldTotalTax ?? '-') as unknown as number,
      rateableValue: (item.rateableValue ?? '-') as unknown as number,
      capitalValue: (item.capitalValue ?? '-') as unknown as number,
