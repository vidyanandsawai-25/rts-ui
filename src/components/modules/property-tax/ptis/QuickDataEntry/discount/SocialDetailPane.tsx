...
import { SocialSelectPrompt, SocialDisabledPrompt, DisabledBanner } from './SocialPanePlaceholders';
...
export const SocialDetailPane: React.FC<SocialDetailPaneProps> = ({
  ...
  const isPhotoInvalid =
    !!errorMsg &&
    ((errorMsg.includes('required') && !data.documentGuid) || errorMsg === docRequiredMsg);
  ...
  if (errorMsg && !isPhotoInvalid) {
    if ((data.dataType || '').toUpperCase() === 'BIT') {
      ...
      errorMsg === (t('property.validation.invalidCharacters') || 'Contains invalid characters.')
    ) {
      const textValueInvalid =
        (data.dataType || '').toUpperCase() === 'VARCHAR' &&
        data.textValue &&
        !/^[^<>]*$/.test(String(data.textValue));
      if (textValueInvalid) {
        isValueInvalid = true;
      } else {
        isRemarkError = true;
        ...
        <div className="pb-3 border-b border-blue-50 flex items-start justify-between gap-4">
          ...
          {onDeleteSocialDetail && isSocialDetailFilled && isUpdateCase && !isDisabled && (
            ...
            >
              ...
              {t('discount.deleteSocialDetail') || 'Delete Social & Data'}
            </button>
          )}
        </div>
