...
export default function OwningDepartmentForm({
  initialData,
}: {
  id?: number | null;
  ...
  useEffect(() => {
    if (open) {
      setTimeout(() => {
        ...
        } else if (!isEdit && formFieldsRef.current?.nameRef?.current) {
          formFieldsRef.current.nameRef.current.focus();
        }
      }, 150);
    }
  }, [open, isEdit]);
    ...
    <Drawer
      ...
      footer={
        <>
          ...
          <SaveButton label={isEdit ? t("form.actions.update") : t("form.actions.save")} type="submit" form="owning-department-form" isLoading={isSubmitting} />
        </>
      }
    >
      <form id="owning-department-form" onSubmit={handleSubmit} className="flex h-full flex-col gap-6 bg-[#F8FAFF] p-5">
