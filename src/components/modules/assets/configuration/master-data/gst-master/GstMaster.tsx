...
import { getGstMasterColumns } from "./GstMasterColumns";
...
export function GstMaster({ data, pageNumber, pageSize, totalCount, totalPages, sortBy, sortOrder, searchTerm }: GstMasterProps) {
  ...
  const columns = React.useMemo(() => getGstMasterColumns({
    t,
    tCommon,
    sortBy,
    sortOrder,
  ...
  const handleDelete = React.useCallback((row: GstMaster) => {
    confirm({
      variant: "delete",
      ...
      <div className="space-y-4">
        <div className="flex items-center justify-between gap-4 rounded-xl border bg-white p-4 shadow-sm">
          ...
          <div className="flex w-full max-w-xl items-center gap-3">
            ...
            />
            <AddButton label={t("add")} onClick={() => router.push(`/${locale}/assets/configuration/master-data/gst-master/add`)} />
          </div>
        </div>
        <MasterTable<GstMaster>
          columns={columns}
          data={data}
          loading={isPending}
          height="lg"
          ...
          renderActions={(row) => <><EditButton aria-label={t("edit")} onClick={() => router.push(`/${locale}/assets/configuration/master-data/gst-master/edit/${row.id}`)} /><DeleteButton aria-label={t("delete")} onClick={() => handleDelete(row)} /></>}
