...
export default function PenaltyRuleMasterForm({
  initialData,
}: {
  id?: number | null;
  ...
  useEffect(() => {
    if (open) {
      setTimeout(() => {
        ...
        } else if (!isEdit && formFieldsRef.current?.penaltyCodeRef?.current) {
          formFieldsRef.current.penaltyCodeRef.current.focus();
        }
      }, 150);
    }
  }, [open, isEdit]);
    ...
    <Drawer
      ...
      footer={
        <>
          ...
          <SaveButton label={isEdit ? t("form.actions.update") : t("form.actions.save")} type="submit" form="penalty-rule-master-form" isLoading={isSubmitting} />
        </>
      }
    >
      <form id="penalty-rule-master-form" onSubmit={handleSubmit} className="flex h-full flex-col gap-6 bg-[#F8FAFF] p-5">
