...
import {
  enrichRenterDetailsFromSession,
  mapInitialRenterFormData,
} from '@/lib/utils/renter/renter-form-mapper';
import { mapCustomDateRangesToPostPayload } from '@/lib/utils/renter/renterUtils';
import { toast } from 'sonner';
import { RenterFormData, RenterPayload } from '@/types/renter/renter.types';
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      const finalPropertyId =
        !isNaN(parsedPropertyId) && parsedPropertyId > 0 ? parsedPropertyId : 1;
      ...
      const payload: RenterPayload = {
        ...
        nonCalculateRentMonthly:
          renterDetails.selfDeclarationAmount &&
      ...
      if (saveAction && finalPropertyDetailsId !== undefined) {
          ...
          const errorMsg =
            typeof result?.error === 'object' && result?.error?.message
              ...
              : typeof result?.error === 'string'
                ? result.error
                : 'Failed to save renter details to server';
        ...
        if (result?.data) {
          ...
          finalPayload = {
            ...
            nonCalculateRentMonthly:
              payload.nonCalculateRentMonthly ?? savedFloor?.nonCalculateRentMonthly,
          };
        }
      }

      const activeFloorId =
        pickValidPropertyDetailsId(finalPayload.id, finalPayload.propertyDetailsId, floorId) ??
        floorId;
      ...
      try {
        ...
        if (sessionForm) {
          ...
          if (
            String(parsed.id) === String(activeFloorId) ||
            (activeFloorId === 'new' && (!parsed.id || parsed.id === 'new'))
          ) {
            const updatedForm = {
              ...parsed,
              renter: 'Yes',
              renterName:
                (finalPayload as any).renterName ||
                (finalPayload as any).renterNameEnglish ||
                parsed.renterName ||
                '',
              agreementFromDate:
                (finalPayload as any).agreementFromDate || parsed.agreementFromDate || null,
              agreementToDate:
                (finalPayload as any).agreementToDate || parsed.agreementToDate || null,
              ...
              nonCalculateRentMonthly:
                (finalPayload as any).nonCalculateRentMonthly ||
                parsed.nonCalculateRentMonthly ||
                0,
