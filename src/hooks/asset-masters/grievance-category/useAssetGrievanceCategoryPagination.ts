...
interface UseAssetGrievanceCategoryPaginationProps {
  pageNumber: number;
  pageSize: number;
  totalCount: number;
  locale: string;
...
export function useAssetGrievanceCategoryPagination({
  pageNumber,
  pageSize,
  totalCount,
  locale,
  ...
  sortBy,
  sortOrder,
}: UseAssetGrievanceCategoryPaginationProps) {
  ...
  const buildUrl = useCallback(
    (page: number, size: number, query?: string, sort?: string, order?: string) => {
      ...
      return `/${locale}/assets/configuration/master-data/grievance-category-master?${params.toString()}`;
    },
    [locale]
  );
