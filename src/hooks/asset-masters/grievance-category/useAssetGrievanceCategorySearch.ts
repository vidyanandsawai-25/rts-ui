...
interface UseAssetGrievanceCategorySearchProps {
  pageSize: number;
  locale: string;
  sortBy?: string;
  sortOrder?: string;
...
export function useAssetGrievanceCategorySearch({
  pageSize,
  locale,
  sortBy,
  sortOrder,
}: UseAssetGrievanceCategorySearchProps) {
  ...
  useEffect(() => {
    ...
    router.push(`/${locale}/assets/configuration/master-data/grievance-category-master?${params.toString()}`);
