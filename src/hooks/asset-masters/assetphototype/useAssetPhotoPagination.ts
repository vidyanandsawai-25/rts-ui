...
interface UseAssetPhotoPaginationProps {
  pageNumber: number;
  pageSize: number;
  totalCount: number;
  locale: string;
...
export function useAssetPhotoPagination({
  pageNumber,
  pageSize,
  totalCount,
  locale,
  ...
  const buildUrl = useCallback(
    (page: number, size: number, query?: string, sort?: string, order?: string) => {
      ...
      return `/${locale}/assets/configuration/master-data/asset-photo-type?${params.toString()}`;
    },
    [locale]
  );
