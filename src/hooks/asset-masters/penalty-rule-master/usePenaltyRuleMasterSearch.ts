...
interface UsePenaltyRuleMasterSearchProps {
  pageSize: number;
  locale: string;
  sortBy?: string;
  sortOrder?: string;
...
export function usePenaltyRuleMasterSearch({
  pageSize,
  locale,
  sortBy,
  sortOrder,
}: UsePenaltyRuleMasterSearchProps) {
  ...
  useEffect(() => {
    ...
    router.push(`/${locale}/assets/configuration/master-data/penalty-rule-master?${params.toString()}`);
