...
interface UseAssetGrievanceRemarkPaginationProps {
  pageNumber: number;
  pageSize: number;
  totalCount: number;
  locale: string;
...
export function useAssetGrievanceRemarkPagination({
  pageNumber,
  pageSize,
  totalCount,
  locale,
  ...
  sortBy,
  sortOrder,
}: UseAssetGrievanceRemarkPaginationProps) {
  ...
  const buildUrl = useCallback(
    (page: number, size: number, query?: string, sort?: string, order?: string) => {
      ...
      return `/${locale}/assets/configuration/master-data/grievance-remark-master?${params.toString()}`;
    },
    [locale]
  );
