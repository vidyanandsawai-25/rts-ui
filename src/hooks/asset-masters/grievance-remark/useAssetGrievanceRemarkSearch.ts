...
interface UseAssetGrievanceRemarkSearchProps {
  pageSize: number;
  locale: string;
  sortBy?: string;
  sortOrder?: string;
...
export function useAssetGrievanceRemarkSearch({
  pageSize,
  locale,
  sortBy,
  sortOrder,
}: UseAssetGrievanceRemarkSearchProps) {
  ...
  useEffect(() => {
    ...
    router.push(`/${locale}/assets/configuration/master-data/grievance-remark-master?${params.toString()}`);
