...
interface UseRtsDepartmentPaginationProps {
  pageNumber: number;
  pageSize: number;
  totalCount: number;
  locale: string;
...
export function useRtsDepartmentPagination({
  pageNumber,
  pageSize,
  totalCount,
  locale,
  ...
  sortOrder = "",
  startTransition,
}: UseRtsDepartmentPaginationProps) {
  ...
  const buildUrl = useCallback(
    (page: number, size: number, search: string, sortCol: string, sortDir: string) => {
      ...
      return `/${locale}/rts/departments${queryStr ? `?${queryStr}` : ""}`;
    },
    [locale]
  );
