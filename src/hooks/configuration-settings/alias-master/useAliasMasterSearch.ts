...
interface UseAliasMasterSearchProps {
  pageSize: number;
  locale: string;
  sortBy?: string;
  sortOrder?: string;
...
export function useAliasMasterSearch({
  pageSize,
  locale,
  sortBy,
  sortOrder,
}: UseAliasMasterSearchProps) {
  ...
  useEffect(() => {
    ...
    router.push(`/${locale}/configuration-settings/alias-master?${params.toString()}`);
